...
class HDFC:
    ...
    def validation(self):
        try:
            ...
            # tables = camelot.read_pdf(self.file_name, flavor='lattice', pages="all",password=self.passwd)
            self.valid = True
        except Exception as e:
    ...
    def extract(self):
        num_pages = len(PdfReader(self.file_name).pages)
        print("NO.Of Pages In Pdf is", num_pages)
        print("\n")

        for i in range(1, num_pages):
            i = str(i)
            all_df = tb.read_pdf(self.file_name, pages=i,
                                 multiple_tables=True, stream=True)
            df = pd.DataFrame(all_df[0])
            i = int(i)
            if i >= 2:
                self.df_total = self.df_total.transpose().reset_index(drop=True).transpose()
                df = df.transpose().reset_index(drop=True).transpose()
                dfs = [self.df_total, df]
                self.df_total = pd.concat(dfs, axis=0, ignore_index=True)
            if i == 1:
                self.df_total = self.df_total.append(df, ignore_index=True)

        all_df = tb.read_pdf(self.file_name, pages="1",
                             multiple_tables=True, stream=True)
        df = pd.DataFrame(all_df[0])
        df = df.iloc[:0]

        # extra
        # df_total=df_total.iloc[0:,0:-1]

        head = pd.Series(list(df.columns), index=self.df_total.columns)
        self.df_total = self.df_total.append(head, ignore_index=True)
        self.df_total = self.df_total.apply(np.roll, shift=1)
        self.df_total = self.df_total.reset_index(drop=True)

        self.Index()

        df_copy = self.df_total.copy()
        df_copy = df_copy.replace('', np.nan, regex=True)
        df_copy[self.desc_index] = df_copy[self.desc_index].astype(str)
        for i in range(1, len(df_copy)):
            x = df_copy.iloc[i, self.date_index]
            detail = df_copy.iloc[i, self.desc_index]
            if pd.isnull(x):
                pass
            else:
                for j in range(1, 10):
                    nindex = i+j
                    if nindex < len(self.df_total):
                        y = df_copy.iloc[nindex, self.date_index]
                        if pd.isnull(y):
                            add = df_copy.iloc[nindex, self.desc_index]
                            detail = detail+add
                        else:
                            break
            df_copy.iloc[i, self.desc_index] = detail

        df_copy = df_copy[pd.notnull(df_copy[self.date_index])]
        df_copy = df_copy.reset_index(drop=True)

        self.df_total = df_copy.copy()
    ...
    def EOD(self):
        ...
        def MAB(month):
            for i in range(len(z)):
                ...
                year = list(x[x['Month'] == month]['Year'])
                temp = temp[['Day', 'EOD']]
                ...
                temp.columns.values[1] = mlist[month]+'-'+str(year[0])
                temp = temp.set_index(['Day'])
            return temp

        # merge all months
        mon = x['Month'].unique().tolist()
        if 0 in mon:
            mon.remove(0)
        first = mon[0]
        mab = pd.merge(day, MAB(first), how="left", on=["Day"])
        mon.pop(0)
        for i in mon:
            s = MAB(i)
    ...
    def Index(self):
        ...
        bal = ['Balance', 'Balance (INR )', 'Closing Balance', 'BALANCE']
    ...
    def WITHDRAWL(self):
        df_temp3 = self.df_total.copy()
        df_ft = pd.DataFrame()
        match3 = ['withdraw', 'ATM', 'wdl', 'Withdrawal Amt.']
        templ3 = [0]
        for i in range(2, len(df_temp3)):
            for x in match3:
                if re.search(x, df_temp3[self.desc_index].iloc[i], re.IGNORECASE):
...
# enddate = '29/03/2020'
# HDFC('hdfc.pdf', passwd, startdate, enddate)
