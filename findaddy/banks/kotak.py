...
class KOTAK:
    ...
    def validation(self):
        try:
            ...
            tables = camelot.read_pdf(
                self.file_name, flavor='stream', pages="all", password=self.passwd)
            self.valid = True
        except Exception as e:
    ...
    def extract(self):
        ...
        for i in range(num_pages):
            ...
            if i == 0:
                try:
                    ...
                    m = m+z
                    tables = camelot.read_pdf(
                        self.file_name, password=self.passwd, flavor='stream', pages=z, table_regions=['28,613,567,93'])
                    c = tables[0].df
                    c.replace({r'\n': ' '}, regex=True, inplace=True)
                    # c=c.iloc[1:]
                    self.df_total = self.df_total.append(c, ignore_index=True)
                except:
                    print('error')
            else:
                try:
        ...
        df_copy = df_copy[pd.notnull(df_copy[self.bal_index])]
    ...
    def Index(self):
        ...
        desc = ['Description', 'Particular', 'Details', 'Narration',
                'Particulars', 'Transaction Remarks', 'NARRATION', 'TRANSACTION DETAILS']
        debit = ['Debit', 'Withdrawals', 'Withdrawal (Dr)', 'Withdrawal(Dr)', 'Withdrawal Amount (INR )',
                 'Withdrawal Amt.', 'WITHDRAWAL (DR)', 'WITHDRAWAL(DR)', 'WITHDRAWS', 'DEBIT(₹)']
        credit = ['Credit', 'Deposits', 'Deposit(Cr)', 'Deposit (Cr)', 'Deposit Amount (INR )',
                  'Deposit Amt.', 'DEPOSIT (CR)', 'DEPOSIT(CR)', 'DEPOSIT', 'CREDIT(₹)']
        date = ['Date', 'Tran Date', 'TRANSACTION', 'Transaction Date',
...
# enddate='31/03/2021'
# KOTAK('2712854827_statement.pdf',passwd,startdate,enddate)
