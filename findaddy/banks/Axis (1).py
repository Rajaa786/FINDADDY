...
class AXIS:
    def __init__(self, file_name, password, startdate, enddate):
        ...
        self.Excel_File = pd.ExcelWriter("BankStatement.xlsx", engine="openpyxl")
        self.show()
    ...
    def extract(self):
        ...
        self.df_total.to_excel(self.Excel_File, sheet_name="Transaction", index=False)
    ...
    def EOD(self):
        ...
        x[["Day", "Month", "Year"]] = x.apply(month, axis=1, result_type="expand")
        ...
        mab.to_excel(self.Excel_File, sheet_name="open", index=False)
        # self.Excel_File.save()
        # print('EOD')
        print(open_bal)
        return mab
    ...
    def QuaterlyAvg(self, mab):
        ...
        if colwidth > 3:
            ...
            for i in range(tot):
                ...
                ind += 1
                total_sum = (
                    quatavg.iloc[:31, tot_index - 1].sum()
                    + quatavg.iloc[:31, tot_index - 2].sum()
                    + quatavg.iloc[:31, tot_index - 3].sum()
                )
                quatavg[index].iloc[31] = total_sum
                quatavg[index].iloc[32] = (
                    quatavg.iloc[:31, tot_index - 1].mean()
                    + quatavg.iloc[:31, tot_index - 2].mean()
                    + quatavg.iloc[:31, tot_index - 3].mean()
                )
                quatavg.to_excel(
    ...
    def HalflyAvg(self, mab):
        ...
        if colwidth > 6:
            ...
            for i in range(tot):
                ...
                ind += 1
                total_sum = (
                    halfavg.iloc[:31, tot_index - 1].sum()
                    + halfavg.iloc[:31, tot_index - 2].sum()
                    + halfavg.iloc[:31, tot_index - 3].sum()
                    + halfavg.iloc[:31, tot_index - 4].sum()
                    + halfavg.iloc[:31, tot_index - 5].sum()
                    + halfavg.iloc[:31, tot_index - 6].sum()
                )
                halfavg[index].iloc[31] = total_sum
                halfavg[index].iloc[32] = (
                    halfavg.iloc[:31, tot_index - 1].mean()
                    + halfavg.iloc[:31, tot_index - 2].mean()
                    + halfavg.iloc[:31, tot_index - 3].mean()
                    + halfavg.iloc[:31, tot_index - 4].mean()
                    + halfavg.iloc[:31, tot_index - 5].mean()
                    + halfavg.iloc[:31, tot_index - 6].mean()
                )
                halfavg.to_excel(
    ...
    def YearlyAvg(self, mab):
        ...
        if colwidth > 12:
            ...
            for i in range(tot):
                ...
                ind += 1
                total_sum = (
                    yearavg.iloc[:31, tot_index - 1].sum()
                    + yearavg.iloc[:31, tot_index - 2].sum()
                    + yearavg.iloc[:31, tot_index - 3].sum()
                    + yearavg.iloc[:31, tot_index - 4].sum()
                    + yearavg.iloc[:31, tot_index - 5].sum()
                    + yearavg.iloc[:31, tot_index - 6].sum()
                    + yearavg.iloc[:31, tot_index - 7].sum()
                    + yearavg.iloc[:31, tot_index - 8].sum()
                    + yearavg.iloc[:31, tot_index - 9].sum()
                    + yearavg.iloc[:31, tot_index - 10].sum()
                    + yearavg.iloc[:31, tot_index - 11].sum()
                    + yearavg.iloc[:31, tot_index - 12].sum()
                )
                yearavg[index].iloc[31] = total_sum
                yearavg[index].iloc[32] = (
                    yearavg.iloc[:31, tot_index - 1].mean()
                    + yearavg.iloc[:31, tot_index - 2].mean()
                    + yearavg.iloc[:31, tot_index - 3].mean()
                    + yearavg.iloc[:31, tot_index - 4].mean()
                    + yearavg.iloc[:31, tot_index - 5].mean()
                    + yearavg.iloc[:31, tot_index - 6].mean()
                    + yearavg.iloc[:31, tot_index - 7].mean()
                    + yearavg.iloc[:31, tot_index - 8].mean()
                    + yearavg.iloc[:31, tot_index - 9].mean()
                    + yearavg.iloc[:31, tot_index - 10].mean()
                    + yearavg.iloc[:31, tot_index - 11].mean()
                    + yearavg.iloc[:31, tot_index - 12].mean()
                )
    ...
    def CASH_DEPOSITS(self):
        ...
        # keywords
        match = ["Cash & Deposit Machine", "CASH DEPOSIT", "CASH DEP", "CDM", "CDS"]
    ...
    def WITHDRAWL(self):
        ...
        if len(templ3) == 1:
            ...
            df_ft.to_excel(self.Excel_File, sheet_name="Withdrawal", index=False)
    ...
    def FUND_TRANSFER(self):
        ...
        if len(templ3) == 1:
            ...
            df_ft.to_excel(self.Excel_File, sheet_name="FundTransfer", index=False)
...
AXIS("Bank Statement Axis Full- MUKE019031687.pdf", "MUKE019031687", " 01/04/2021", "13/03/2022")
