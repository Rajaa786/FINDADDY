...
class ICICI:
    ...
    def extract(self):
        pdf_reader = PyPDF2.PdfFileReader(self.file_name)
        num_pages = pdf_reader.numPages
        print("NO.Of Pages In Pdf ", num_pages)
        print("\n")
        m = "table"
        for i in range(num_pages):
            ...
            if i == 0:
                try:
                    ...
                    m = m+z
                    tables = camelot.read_pdf(
                        self.file_name, flavor='stream', pages=z, table_regions=['31,950,1000,50'])
                    c = tables[0].df
                    c = c.iloc[1:]
                    c.replace({r'\n': ' '}, regex=True, inplace=True)
                    # c.to_excel(writer, sheet_name=m, index=False)
                ...
                try:
                    ...
                    m = m+z
                    tables = camelot.read_pdf(
                        self.file_name, flavor='stream', pages=z, table_regions=['31,1400,1000,78'])
                    c = tables[0].df
                    # c = c.iloc[1:]
                    c.replace({r'\n': ' '}, regex=True, inplace=True)
                    # c.to_excel(writer, sheet_name=m, index=False)
...
# startdate = '01/3/2020'
# enddate = '18/10/2020'
# ICICI('ICICI Bank 6 Month Statement.pdf', passwd, startdate, enddate)
